...
import { ResilientLLMError } from "./error/ResilientLLMError.js";
...
class ResilientOperation {
    ...
    constructor({
        ...
        // Metrics collection (opt-in for observability)
    ...
    /**
     * Get collected runtime metrics (returns null when collectMetrics is disabled)
     * @returns {Object|null}
     */
    getRuntimeMetrics() {
     ...
     * @returns {this} - Returns this instance for method chaining
     */
    withCache(isEnabled = true, cacheStore) {
    ...
    async _executeBasic(asyncFn, config, ...args) {
        ...
        while (retryAttempt <= config.retries) {
            try {
                ...
                if (this._collectMetrics && rateLimitResult) {
                ...
                if (err.name === 'OversizedRequestError') {
                    console.log(`[ResilientOperation][${this.id}] Oversized request error: ${err.message}`);
                    throw err;
                }
                ...
                const willRetry = this._shouldRetry(err) && retryAttempt < config.retries;
                ...
                if (this._collectMetrics) {
                    ...
                    const cause = ResilientLLMError.serializeCause(err?.cause, { excludeKeys: ['stack'] });
                    if (cause) retryEntry.cause = cause;
                    this._runtimeMetrics.retries.push(retryEntry);
                    this._runtimeMetrics.circuitBreaker = status;
                }
                ...
                if (!willRetry) {
                    ...
                    if (this._isNetworkConnectivityError(err)) {
                        console.warn(`[ResilientOperation][${this.id}] This may be due to network connectivity. Please check your internet connection.`);
                    }
                    throw err;
                }
    ...
    /**
     * Heuristic: true if the error appears to be due to network connectivity (transient).
     * @param {Error} err - The error object
     * @returns {boolean}
     * @private
     */
    _isNetworkConnectivityError(err) {
        if (!err) return false;
        ...
        if (message === 'fetch failed' || /timeout|connection refused|network|econnreset|enotfound|etimedout/i.test(message)) return true;
        return false;
    }
