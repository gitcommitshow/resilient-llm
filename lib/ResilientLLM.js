...
class ResilientLLM {
    ...
    async chat(conversationHistory, llmOptions = {}) {
        ...
        const hasExplicitAiService = llmOptions?.aiService !== undefined && llmOptions?.aiService !== null;
        let aiService = llmOptions?.aiService || this.aiService;
        let model = llmOptions?.model || this.model;

        // Accept model in either "model-id" or canonical "provider/model-id" format.
        // This minimizes migration work from ecosystems that use provider/model ids.
        if (typeof model === 'string' && model.includes('/')) {
            const slashIndex = model.indexOf('/');
            const modelProviderPrefix = model.slice(0, slashIndex).replace(/\s+/g, '').toLowerCase();
            const providerModelId = model.slice(slashIndex + 1);
            const normalizedAiService = (aiService || '').replace(/\s+/g, '').toLowerCase();

            if (hasExplicitAiService) {
                // Respect explicit aiService; strip prefix only when it matches.
                if (modelProviderPrefix === normalizedAiService && providerModelId) {
                    model = providerModelId;
                }
            } else {
                // No explicit aiService for this request: if prefix is a known provider, use it.
                const providerFromModel = ProviderRegistry.get(modelProviderPrefix);
                if (providerFromModel && providerModelId) {
                    aiService = modelProviderPrefix;
                    model = providerModelId;
                }
            }
        }
        ...
        const isReasoning = ProviderRegistry.isReasoningModel(aiService, model);
        if (isReasoning) {
        ...
        // Use capability API for temperature/topP; skip when explicitly unsupported
        const tempSupported = ProviderRegistry.isTemperatureSupported(aiService, model);
