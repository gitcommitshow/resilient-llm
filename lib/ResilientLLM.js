...
class ResilientLLM {
    ...
    async chat(conversationHistory, llmOptions = {}, observabilityOptions = {}) {
        ...
        try {
            ...
            */
            ...
            const wrapped = ResilientLLMError.from(error, {
                provider: aiService,
                metadata: returnOperationMetadata ? (metadata ?? null) : null,
            });
            throw wrapped;
        }
    }
    ...
    static _captureHttpMetadata(metadata, apiUrl, response, durationMs, error) {
        ...
        if (error?.cause) {
            const serialized = ResilientLLMError.serializeCause(error.cause, { excludeKeys: ['stack'] });
            if (serialized) httpEntry.cause = serialized;
        }
        metadata.http = httpEntry;
