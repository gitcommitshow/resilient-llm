...
class ResilientLLM {
    ...
    async chat(conversationHistory, llmOptions = {}) {
        ...
        const apiKey = ProviderRegistry.getApiKey(aiService);
        
        // Validate API key for providers that require it
        if (!apiKey && !providerConfig.authConfig?.optional) {
        ...
        if (apiKey && providerConfig.authConfig?.type === 'query') {
            const separator = apiUrl.includes('?') ? '&' : '?';
            const paramName = providerConfig.authConfig.queryParam || 'key';
            apiUrl = `${apiUrl}${separator}${paramName}=${encodeURIComponent(apiKey)}`;
        }
