...
class ProviderRegistry {
    ...
    // Prevent serialization to make it harder to access. Use WeakMap.
    static #apiKeys = new WeakMap();
    ...
    static configure(providerName, config) {
        ...
        // Store API key securely using WeakMap with provider config as key
        if (apiKeyToStore) {
            this.#apiKeys.set(merged, apiKeyToStore);
        }
    ...
    static getApiKey(providerName) {
        ...
        // Priority 1: Direct apiKey from WeakMap (highest priority)
        const storedKey = this.#apiKeys.get(provider);
        if (storedKey) {
            return storedKey;
        }
     ...
     */
    static buildAuthHeaders(providerName, apiKey, defaultHeaders = {}) {
        this.init();
        // Normalize provider name (get() will also normalize, but be explicit)
        providerName = this.#normalizeProviderName(providerName);
        const provider = this.get(providerName);
    ...
    static reset() {
        ...
        this.#modelsCache.clear();
        // WeakMap will automatically clean up when provider objects are GC'd
