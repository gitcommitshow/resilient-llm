 ...
 *
 * Provider and model identifiers (standard; consistent for all consumers and catalogs):
 * - Provider ID: lowercase, no spaces (e.g. openai, anthropic, google, ollama). Use this in configure(), getModels(), capability API, etc. External catalogs are matched by normalizing their provider keys to this form.
 * - Model ID: the provider's native identifier (e.g. gpt-4o, claude-3-5-sonnet-20241022). No library-specific mapping; same IDs the provider API uses.
 */

/**
 * Unified Model Schema - single merged record per (provider, modelId).
 * Populated from provider models API (highest priority), reference catalogs
 * (e.g. models.dev, lower priority), and heuristics (last resort).
 * Consumers should use the capability API (isReasoningModel, isToolCallSupported, etc.)
 ...
 * @property {number} [contextWindow] - Maximum context/input tokens
 * @property {number} [maxOutputTokens] - Maximum output tokens
 * @property {boolean} [isReasoning] - Uses reasoning params (reasoning_effort, max_completion_tokens)
 * @property {boolean} [toolCall] - Supports tool/function calling
 * @property {boolean} [temperature] - Supports temperature parameter
 * @property {boolean} [structuredOutput] - Supports dedicated structured-output feature
 * @property {boolean} [attachment] - Supports file attachments
 * @property {Object} [cost] - Token cost rates (USD per million tokens)
 * @property {number} [cost.input] - Input cost per 1M tokens
 * @property {number} [cost.output] - Output cost per 1M tokens
 * @property {number} [cost.reasoning] - Reasoning token cost per 1M tokens
 * @property {Object} [modalities] - Supported input/output modalities
 * @property {string[]} [modalities.input] - e.g. ['text','image','audio']
 * @property {string[]} [modalities.output] - e.g. ['text']
 * @property {string} [status] - 'alpha' | 'beta' | 'deprecated' | undefined (GA)
 * @property {string} [_source] - Origin of the record: 'provider', 'catalog', or 'merged'
 * @property {any} raw - Full raw API response for this model (from provider)
 */
...
class ProviderRegistry {
    ...
    static #catalogCache = null; // Raw parsed JSON from the catalog (e.g. models.dev)
    static #catalogFetchedAt = null; // Timestamp of last catalog fetch
    static #catalogTtlMs = 24 * 60 * 60 * 1000; // 24h default TTL
    static #catalogUrl = 'https://models.dev/api.json';
    static #catalogEnabled = true;
    static #catalogFetchPromise = null; // De-duplicate concurrent fetches
    /** @type {Map<string, string[]>|null} Normalized provider name → catalog top-level keys; built when catalog is loaded */
    static #catalogProviderIndex = null;
    /** Optional override when catalog key does not normalize to our provider ID (e.g. ollama vs "Ollama Cloud"). */
    static #catalogProviderMapOverride = { ollama: ['Ollama Cloud'] };
    ...
    static DEFAULT_PROVIDERS = {
        openai: {
            ...
            /** @type {Array<RegExp|string>} Match model id to treat as reasoning model (regex or prefix string); used for request params (reasoning_effort, max_completion_tokens vs temperature, max_tokens) */
            reasoningModelPatterns: [/^o\d/, /^gpt-5/],
            parseConfig: {
                modelsPath: 'data',
                idField: 'id',
                nameField: 'id',
        ...
        google: {
            ...
            // Endpoint-specific auth configs - easily readable and configurable
            endpointAuthConfigs: {
                '/chat/completions': {
                    type: 'header',
                    headerName: 'Authorization',
    ...
    /**
     * Convert any model identifier into provider-native model id.
     * Accepts either "model-id" or "provider/model-id".
     * @private
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Raw or canonical model id
     * @returns {string}
     */
    static #toProviderModelId(providerName, modelId) {
        if (!modelId || typeof modelId !== 'string') return '';
        const normalizedProvider = this.#normalizeProviderName(providerName);
        const trimmed = modelId.trim();
        const prefix = `${normalizedProvider}/`;
        if (normalizedProvider && trimmed.startsWith(prefix)) {
    ...
    /**
     * Build canonical model id as "provider/model-id".
     * @private
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Raw or canonical model id
     * @returns {string}
     */
    static #toCanonicalModelId(providerName, modelId) {
        const normalizedProvider = this.#normalizeProviderName(providerName);
        const providerModelId = this.#toProviderModelId(normalizedProvider, modelId);
        if (!normalizedProvider || !providerModelId) return '';
        return `${normalizedProvider}/${providerModelId}`;
    }

    /**
     * Get lookup keys for cache compatibility.
     * Prefers canonical "provider/model-id" key but also supports raw model id key.
     * @private
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Raw or canonical model id
     * @returns {string[]}
     */
    static #modelLookupKeys(providerName, modelId) {
        const providerModelId = this.#toProviderModelId(providerName, modelId);
        const canonicalId = this.#toCanonicalModelId(providerName, providerModelId);
        const keys = [canonicalId, providerModelId, modelId].filter(Boolean);
     ...
     * @param {Array<RegExp|string>} [config.reasoningModelPatterns] - Patterns to detect reasoning models (regex or prefix string); model id matching these use reasoning_effort/max_completion_tokens instead of temperature/max_tokens
    ...
    static configure(providerName, config) {
        ...
        const merged = {
            ...
            reasoningModelPatterns: config.reasoningModelPatterns !== undefined ? config.reasoningModelPatterns : existing.reasoningModelPatterns,
     ...
     * @param {string} [apiUrl] - Optional API URL for endpoint-specific auth config detection
    ...
    static buildAuthHeaders(providerName, apiKey = null, defaultHeaders = {}, apiUrl = null) {
        ...
        // Determine which auth config to use based on URL pattern
        ...
        // Check endpointAuthConfigs if URL is provided
        if (apiUrl && provider.endpointAuthConfigs) {
            // Find matching endpoint pattern (most specific match wins)
            const matchingPattern = Object.keys(provider.endpointAuthConfigs)
                .filter(pattern => apiUrl.includes(pattern))
                .sort((a, b) => b.length - a.length)[0]; // Prefer longer/more specific patterns
    ...
    static buildApiUrl(providerName, apiUrl, apiKey = null) {
        ...
        // Check endpointAuthConfigs for endpoint-specific config
    ...
    static async getModel(providerName, modelName, apiKey = null) {
        ...
        // Normalize provider name for consistent lookup
        providerName = this.#normalizeProviderName(providerName);
        const lookupKeys = this.#modelLookupKeys(providerName, modelName);
        // Check cache first
        const cached = this.#modelsCache.get(providerName);
        if (cached) {
        ...
        return models.find(m => lookupKeys.includes(m.id) || lookupKeys.includes(m.providerModelId)) || null;
    }
    ...
    static saveModel(providerName, modelName, modelData) {
        ...
        const providerModelId = this.#toProviderModelId(providerName, modelName);
        const canonicalId = this.#toCanonicalModelId(providerName, providerModelId);
    ...
    static async #fetchModels(providerName, provider, apiKey) {
        ...
        // Build URL with query params if needed (auto-detects endpoint-specific auth config from URL)
        // Pass apiKey so it can be used for query parameter auth
        url = this.buildApiUrl(providerName, url, apiKey);
        
        // Build headers using shared auth header building logic
        // Pass url for endpoint-specific auth config detection
    ...
    static #parseModels(providerName, data) {
        ...
        return models.map(model => {
            ...
            const providerModelId = id || 'unknown';
            const canonicalModelId = this.#toCanonicalModelId(providerName, providerModelId);
            const isReasoning = this.#matchesReasoningPatterns(provider, providerModelId);

            return {
                id: canonicalModelId || `${providerName}/${providerModelId}`,
                providerModelId,
                provider: providerName,
                name: displayName || name || providerModelId,
                contextWindow: contextWindow || undefined,
                isReasoning,
                toolCall: undefined,
                temperature: isReasoning ? false : undefined,
    ...
    /**
     * Check if model id matches provider's reasoning model patterns
     * @private
     * @param {Object} provider - Provider config
     * @param {string} modelId - Model identifier
     * @returns {boolean}
     */
    static #matchesReasoningPatterns(provider, modelId) {
        const patterns = provider?.reasoningModelPatterns;
        const providerModelId = this.#toProviderModelId(provider?.name, modelId);
        if (!patterns || !Array.isArray(patterns) || !providerModelId) return false;
        return patterns.some(p => {
            if (typeof p === 'string') return providerModelId === p || providerModelId.startsWith(p);
            if (p instanceof RegExp) return p.test(providerModelId);
            return false;
        });
    }

    /**
     * Get model from cache only (no API call). Use getModel() for fetch when needed.
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Model identifier
     * @returns {UnifiedModel|null}
     */
    static getCachedModel(providerName, modelId) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        const cached = this.#modelsCache.get(providerName);
        if (!cached || !modelId) return null;
        const lookupKeys = this.#modelLookupKeys(providerName, modelId);
        for (const key of lookupKeys) {
    ...
    // ─── Capability API ──────────────────────────────────────────────────
    // Consumers (ResilientLLM, playground, etc.) should call these only.
    // Resolution order: merged record field → heuristic (e.g. reasoningModelPatterns).
    // The merged record is populated from provider API (first priority),
    // reference catalogs like models.dev (lower priority), then heuristics.

    /**
     * Resolve a capability field from the merged model record, with optional
     * heuristic fallback and a default when nothing is known.
     * @private
     */
    static #resolveCapability(providerName, modelId, field, { heuristic, defaultValue } = {}) {
        const cached = this.getCachedModel(providerName, modelId);
        if (cached && typeof cached[field] === 'boolean') return cached[field];
        if (typeof heuristic === 'function') return heuristic(providerName, modelId);
        return defaultValue ?? undefined;
    }

    /**
     * Check if a model is a reasoning model (uses reasoning_effort / max_completion_tokens
     * instead of temperature / max_tokens).
     * Resolution: merged record → reasoningModelPatterns heuristic → false.
     *
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Model identifier
     * @returns {boolean}
     */
    static isReasoningModel(providerName, modelId) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        return this.#resolveCapability(providerName, modelId, 'isReasoning', {
            heuristic: (pn, mid) => {
                const provider = this.get(pn);
                return this.#matchesReasoningPatterns(provider, mid);
            },
            defaultValue: false
        });
    }
     ...
     * Resolution: merged record → undefined (unknown).
     *
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Model identifier
     * @returns {boolean|undefined} true/false if known, undefined if unknown
     */
    static isToolCallSupported(providerName, modelId) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        return this.#resolveCapability(providerName, modelId, 'toolCall');
    }

    /**
     * Check if a model supports the temperature parameter.
     * Resolution: merged record → infer false for reasoning models → undefined.
     *
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Model identifier
     * @returns {boolean|undefined} true/false if known, undefined if unknown
     */
    static isTemperatureSupported(providerName, modelId) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        return this.#resolveCapability(providerName, modelId, 'temperature', {
            heuristic: (pn, mid) => {
                if (this.isReasoningModel(pn, mid)) return false;
                return undefined;
            }
        });
    }
    ...
    /**
     * Check if a model supports a dedicated structured-output feature.
     * Resolution: merged record → undefined (unknown).
     *
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Model identifier
     * @returns {boolean|undefined} true/false if known, undefined if unknown
     */
    static isStructuredOutputSupported(providerName, modelId) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        return this.#resolveCapability(providerName, modelId, 'structuredOutput');
    }

    /**
     * Get the context window (max input tokens) for a model, if known.
     *
     * @param {string} providerName - Provider identifier
     * @param {string} modelId - Model identifier
     * @returns {number|undefined}
     */
    static getContextLimit(providerName, modelId) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        const cached = this.getCachedModel(providerName, modelId);
        return cached?.contextWindow || undefined;
    }
    ...
    // ─── Reference catalog (e.g. models.dev) ────────────────────────────

    /**
     * Configure the reference catalog.
     * Provider IDs use a single standard: lowercase, no spaces (e.g. openai, anthropic, google, ollama).
     * Model IDs are the provider's native identifier. Catalogs are matched by normalizing their
     * provider keys to this form; use providerMap only when a catalog key does not normalize to your provider ID.
     *
     * @param {Object} options
     * @param {boolean} [options.enabled] - Enable/disable catalog lookups
     * @param {string}  [options.url] - Catalog URL (default: https://models.dev/api.json)
     * @param {number}  [options.ttlMs] - Cache TTL in milliseconds (default: 24h)
     * @param {Object<string, string[]>} [options.providerMap] - Override only when catalog key does not normalize to provider ID: { providerId: ['CatalogKey', ...] }
     */
    static configureCatalog(options = {}) {
        if (options.enabled !== undefined) this.#catalogEnabled = options.enabled;
        if (options.url) this.#catalogUrl = options.url;
        if (options.ttlMs !== undefined) this.#catalogTtlMs = options.ttlMs;
        if (options.providerMap) {
            this.#catalogProviderMapOverride = { ...this.#catalogProviderMapOverride, ...options.providerMap };
        }
    }

    /**
     * Fetch the reference catalog JSON (e.g. models.dev/api.json).
     * Caches in memory with TTL. De-duplicates concurrent calls.
     * @returns {Promise<Object|null>} Raw catalog JSON or null on failure
     */
    static async fetchCatalog() {
        if (!this.#catalogEnabled) return null;

        const now = Date.now();
        if (this.#catalogCache && this.#catalogFetchedAt && (now - this.#catalogFetchedAt) < this.#catalogTtlMs) {
        ...
        this.#catalogFetchPromise = (async () => {
            try {
                ...
                if (!response.ok) {
                    ...
                    return this.#catalogCache; // Return stale cache if available
                }
                this.#catalogCache = await response.json();
                this.#catalogFetchedAt = Date.now();
                this.#buildCatalogProviderIndex();
                return this.#catalogCache;
            } catch (error) {
                console.warn('Catalog fetch error (non-fatal):', error.message);
                return this.#catalogCache; // Return stale cache if available
            } finally {
    ...
    /**
     * Look up a model in the catalog cache (sync, no fetch).
     * @param {string} providerName - Our normalized provider name
     * @param {string} modelId - Model identifier
     * @returns {Object|null} Raw catalog model object or null
     */
    static #lookupCatalogModel(providerName, modelId) {
        if (!this.#catalogCache || !modelId) return null;
        const providerModelId = this.#toProviderModelId(providerName, modelId);
        const canonicalId = this.#toCanonicalModelId(providerName, providerModelId);

        const catalogKeys = this.#catalogProviderKeys(providerName);
        for (const key of catalogKeys) {
            const providerEntry = this.#catalogCache[key];
            if (!providerEntry?.models) continue;
            const model = providerEntry.models[canonicalId] || providerEntry.models[providerModelId] || providerEntry.models[modelId];
    ...
    /**
     * Build index: normalized provider name → list of catalog top-level keys.
     * Enables lookup without a static provider map; new providers work if catalog key normalizes to their ID.
     * @private
     */
    static #buildCatalogProviderIndex() {
        this.#catalogProviderIndex = new Map();
        if (!this.#catalogCache || typeof this.#catalogCache !== 'object') return;
        for (const catalogKey of Object.keys(this.#catalogCache)) {
            const normalized = this.#normalizeProviderName(catalogKey);
            if (!this.#catalogProviderIndex.has(normalized)) {
                this.#catalogProviderIndex.set(normalized, []);
            }
            this.#catalogProviderIndex.get(normalized).push(catalogKey);
        }
    }

    /**
     * Get catalog top-level keys for a provider. Uses override first (when catalog key does not normalize to provider ID), then normalization-derived index.
     * @param {string} providerName - Normalized provider ID (lowercase, no spaces)
     * @returns {string[]}
     */
    static #catalogProviderKeys(providerName) {
        const override = this.#catalogProviderMapOverride?.[providerName];
        if (override?.length) return override;
        return this.#catalogProviderIndex?.get(providerName) ?? [];
    }

    /**
     * Merge catalog data into an existing cached model record.
     * Catalog fields are written only when the record doesn't already have a value
     * (provider data has priority).
     *
     * @param {UnifiedModel} record - Existing model record (mutated in place)
     * @param {Object} catalogModel - Raw catalog model object
     * @returns {UnifiedModel} The mutated record
     */
    static #mergeCatalogIntoRecord(record, catalogModel) {
        if (!catalogModel) return record;
        ...
        setIfMissing('isReasoning', catalogModel.reasoning);
        setIfMissing('toolCall', catalogModel.tool_call);
        setIfMissing('temperature', catalogModel.temperature);
        setIfMissing('structuredOutput', catalogModel.structured_output);
        setIfMissing('attachment', catalogModel.attachment);

        if (catalogModel.limit) {
            setIfMissing('contextWindow', catalogModel.limit.context || catalogModel.limit.input);
            setIfMissing('maxOutputTokens', catalogModel.limit.output);
        }

        if (catalogModel.cost) {
            if (!record.cost) record.cost = {};
            for (const [k, v] of Object.entries(catalogModel.cost)) {
                if (record.cost[k] === undefined) record.cost[k] = v;
            }
        }

        if (catalogModel.modalities) {
            if (!record.modalities) record.modalities = {};
            setIfMissing('modalities', undefined); // ensure object exists
            if (!record.modalities.input && catalogModel.modalities.input) {
                record.modalities.input = catalogModel.modalities.input;
            }
            if (!record.modalities.output && catalogModel.modalities.output) {
                record.modalities.output = catalogModel.modalities.output;
            }
        }

        setIfMissing('status', catalogModel.status);

        if (record._source === 'provider') {
    ...
    /**
     * Enrich all cached models for a provider with catalog data (if available in cache).
     * Also adds catalog-only models that the provider didn't list.
     * Call after getModels() and after fetchCatalog() for best results.
     *
     * @param {string} providerName - Our normalized provider name
     */
    static enrichFromCatalog(providerName) {
        this.init();
        providerName = this.#normalizeProviderName(providerName);
        ...
        for (const [modelId, record] of modelMap) {
            const catalogModel = this.#lookupCatalogModel(providerName, modelId);
            if (catalogModel) {
                this.#mergeCatalogIntoRecord(record, catalogModel);
            }
        }

        // Add catalog-only models (ones the provider API didn't return)
        const catalogKeys = this.#catalogProviderKeys(providerName);
        for (const key of catalogKeys) {
            const providerEntry = this.#catalogCache[key];
            if (!providerEntry?.models) continue;
            for (const [catalogModelId, catalogModel] of Object.entries(providerEntry.models)) {
                const providerModelId = this.#toProviderModelId(providerName, catalogModelId);
                const canonicalModelId = this.#toCanonicalModelId(providerName, providerModelId);
                if (!canonicalModelId || modelMap.has(canonicalModelId)) continue;
                const record = {
                    id: canonicalModelId,
                    providerModelId,
                    provider: providerName,
                    name: catalogModel.name || providerModelId,
                    contextWindow: catalogModel.limit?.context || catalogModel.limit?.input || undefined,
                    maxOutputTokens: catalogModel.limit?.output || undefined,
                    isReasoning: catalogModel.reasoning,
                    ...
                    cost: catalogModel.cost || undefined,
                    modalities: catalogModel.modalities || undefined,
                    status: catalogModel.status || undefined,
                    _source: 'catalog',
                    raw: catalogModel
                };
                modelMap.set(canonicalModelId, record);
            }
        }
    }

    /**
     * Clear the catalog cache (forces re-fetch on next access).
     */
    static clearCatalogCache() {
        this.#catalogCache = null;
        this.#catalogProviderIndex = null;
        this.#catalogFetchedAt = null;
        this.#catalogFetchPromise = null;
    }
